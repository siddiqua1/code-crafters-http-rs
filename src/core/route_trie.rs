use crate::core::routing::form_handler;
...
pub struct RouteTrie<Context: Clone + Copy + 'static> {
...
impl<Context: Clone + Copy + 'static> RouteTrie<Context> {
    fn add_segments(
        &mut self,
        segments: &[Segments],
...
impl<Context: Clone + Copy> Routeable<Context> for RouteTrie<Context> {
    fn new() -> Self {
        return RouteTrie {
    ...
    fn add_route_async(
        ...
        self.add_segments(
            &route.path_segments,
            form_handler!(handler, AsyncRouteHandler),
        )?;
        return Ok(());
    }
    ...
    fn add_route_sync(
        ...
        self.add_segments(
            &route.path_segments,
            form_handler!(handler, SyncRouteHandler),
        )?;
        return Ok(());
    }
...
mod tests {
    ...
    fn adding_new_literal(trie: &mut RouteTrie<PlaceholderContext>, path: &'static str) {
        ...
        assert!(trie
            .add_route(path, form_handler!(thunk, SyncRouteHandler))
            .is_ok());
