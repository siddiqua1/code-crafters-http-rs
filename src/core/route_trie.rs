...
// use crate::core::routing::force_boxed;
// use crate::core::routing::Route;

// use crate::core::routing::{
//     AsyncRouteHandler, Identifiers, RouteHandler, Routeable, Segments, SyncRouteHandler,
// };
// use anyhow::anyhow;
// use anyhow::Result;
...
// pub struct RouteTrie<'a, Context: Clone + Copy + 'static> {
//     literal_routes: HashMap<&'static str, Box<RouteTrie<'a, Context>>>,
//     wild_route: Option<(&'static str, Box<RouteTrie<'a, Context>>)>,
//     pub handler: Option<RouteHandler<'a, Context>>,
// }

// impl<'a, Context: Clone + Copy + 'static> RouteTrie<'a, Context> {
//     fn add_segments(
//         &mut self,
//         segments: &[Segments],
//         handler: RouteHandler<'a, Context>,
//     ) -> Result<()> {
...
//         &self,
//         path: &[&'b str],
//         mut path_values: HashMap<&'static str, &'b str>,
//     ) -> Option<(RouteHandler<'a, Context>, Identifiers<'a>)>
//     where
//         'a: 'b,
//         'b: 'a,
//     {
...
// impl<'a, Context: Clone + Copy + 'static> Routeable<'a, Context> for RouteTrie<'a, Context> {
//     fn new() -> Self {
//         return RouteTrie {
...
//     //     let route = Route::new(path, handler.clone())?;
//     //     self.add_segments(&route.path_segments, handler.clone())?;
//     //     return Ok(());
//     // }
...
//         'b: 'a,
//     {
//         let route = Route::new_async(path, handler.clone())?;
//         self.add_segments(
//             &route.path_segments,
//             RouteHandler::Async(force_boxed(handler)),
//         )?;
//         return Ok(());
//     }
//     fn add_route_async2(
...
//     ) -> Result<()> {
//         let route = Route::new_async2(path, handler.clone())?;
//         self.add_segments(&route.path_segments, RouteHandler::Async(handler))?;
//         return Ok(());
//     }
...
// // 'b: 'a,
//     {
//         let route = Route::new_sync(path, handler)?;
//         self.add_segments(&route.path_segments, RouteHandler::Sync(handler))?;
//         return Ok(());
//     }

//     fn match_route<'b>(&self, path: &'b str) -> Option<(RouteHandler<'b, Context>, Identifiers<'b>)>
//     where
//         // 'a: 'b,
//         'b: 'a,
//     {
...
//     async fn async_thunk<'a>(
//         _req: &'a Request<'a>,
//         _paths: &'a Identifiers<'a>,
...
//         assert!(trie.add_route_sync(path, thunk).is_ok());
//         let out = trie.match_route(path);
//         assert!(out.is_some());
//         let (handler, ids) = out.unwrap();
//         let RouteHandler::Sync(handler) = handler else {
//             panic!("thunk should be sync");
//         };
...
//         assert!(trie.add_route_sync(path, thunk).is_err());
//     }

//     fn adding_new_literal_async(trie: &mut RouteTrie<PlaceholderContext>, path: &'static str) {
//         let test_request: Request = Request::from(RAW_REQUEST).unwrap();
//         assert!(trie.add_route_sync(path, thunk).is_ok());
//         let out = trie.match_route(path);
//         assert!(out.is_some());
